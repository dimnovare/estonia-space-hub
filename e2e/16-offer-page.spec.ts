...
test.describe("Public offer page", () => {
  test("happy path: renders options, requests one, then shows pending confirmation", async ({ page }) => {
    await stubAll(page);
    ...
    await expect(page.getByText("€89 / kuu")).toBeVisible();
    // Concierge note renders.
    await expect(page.getByText(/mõlemad partnerid saavad/i)).toBeVisible();

    // Request the first option → confirm the pending request.
    ...
    // Pending-confirmation state: banner + request badge; no more request buttons.
    await expect(page.getByText(/soov saadetud/i)).toBeVisible({ timeout: 10000 });
    await expect(page.getByText(/ruumly kinnitab saadavuse partneriga/i)).toBeVisible();
    await expect(page.getByText("Sinu soov", { exact: true })).toBeVisible();
    await expect(page.getByRole("button", { name: /küsi seda pakkumist/i })).toHaveCount(0);
  });

  test("already-requested offer renders pending confirmation state without request buttons", async ({ page }) => {
    await stubAll(page);
    await stubPublicOffer(page, {
      offer: publicOffer({ status: "chosen", chosenOptionId: "opt-2" }),
    });
    await page.goto("/et/offer/tok-abc123");
    await expect(page.getByText(/soov saadetud/i)).toBeVisible({ timeout: 15000 });
    await expect(page.getByText(/ruumly kinnitab saadavuse partneriga/i)).toBeVisible();
    await expect(page.getByText("Sinu soov", { exact: true })).toBeVisible();
    await expect(page.getByRole("button", { name: /küsi seda pakkumist/i })).toHaveCount(0);
  });
  ...
  test("choose failure (stale option 400) shows a visible banner, not a silent close", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: /küsi seda pakkumist/i }).first()).toBeVisible();
  });
