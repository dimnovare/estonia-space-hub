...
import { generateOrderEmailPreview, getEmailLabels } from "@/lib/constants";
    ...
    <div className="rounded-xl border border-border overflow-hidden">
      <div className="bg-secondary px-4 py-3 border-b border-border">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <span className="font-medium text-foreground">{l.from}:</span> info@ruumly.eu
        </div>
        <div className="flex items-center gap-2 text-xs text-muted-foreground mt-1">
          <span className="font-medium text-foreground">{l.to}:</span> {order.supplierName} &lt;{
            order.integrationType === "email" ? order.customerEmail.replace(/.*@/, `${order.supplierName.toLowerCase().replace(/\s/g, "")}@`) : "api@partner.ee"
          }&gt;
        </div>
        <div className="flex items-center gap-2 text-xs text-muted-foreground mt-1">
          <span className="font-medium text-foreground">{l.subject}:</span> #{order.id} — {order.listingTitle}
        </div>
      </div>
